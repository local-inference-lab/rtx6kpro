...
> Historical context labels in the decode matrices below came from an older
> prompt generator. In this run, requested `16k/32k/64k/128k` corresponded to
> about `10.5k/20.8k/41.5k/82.8k` actual prompt tokens. The old `128k` rows are
> therefore marked as `~83k actual`; use `kimi-k26-v2.md` or newer for true
> 128k-token comparisons.
...
| ~83k actual (old `128k` label) | 73.6 | 109.3 | 117.3 | 159.0 | 191.3 | 301.1 | 215.2 | 395.5 |
...
| ~83k actual (old `128k` label) | 56.7 | 99.3 | 135.1 | 191.0 | 238.3 | 286.5 | 318.2 | 381.3 |
...
| ~83k actual (old `128k` label) | 73.7 | 93.4 | 118.2 | 155.0 | 198.7 | 242.3 | 315.5 | 424.4 |
...
| ~83k actual (old `128k` label) | 61.6 | 93.5 | 139.1 | 174.9 | 206.6 | 286.0 | 317.9 | 381.4 |
...
| ~83k actual (old `128k` label) | 77.6 | 108.4 | 141.1 | 194.8 | 268.2 | 368.9 | 485.8 | 464.2 |
...
| ~83k actual (old `128k` label) | 66.6 | 97.4 | 147.0 | 198.8 | 254.5 | 349.8 | 508.7 | 635.4 |
...
> These historical matrices used the same older prompt generator as the main
> tables above. Rows marked `~83k actual` were originally labeled `128k`.
...
| ~83k actual (old `128k` label) | 66.6 | 105.4 | 121.2 | 152.6 | 186.5 | 211.1 | 189.1 | 374.1 |
...
| ~83k actual (old `128k` label) | 55.4 | 97.3 | 131.3 | 190.7 | 222.6 | 286.0 | 317.9 | 380.5 |
...
| ~83k actual (old `128k` label) | 67.6 | 87.4 | 112.4 | 152.0 | 175.8 | 213.5 | 247.7 | 383.9 |
...
| ~83k actual (old `128k` label) | 60.3 | 92.8 | 135.1 | 174.7 | 206.7 | 254.6 | 318.0 | 379.7 |
...
| ~83k actual (old `128k` label) | 75.4 | 99.4 | 135.2 | 197.9 | 252.2 | 312.5 | 417.6 | 416.2 |
...
| ~83k actual (old `128k` label) | 65.6 | 95.5 | 146.8 | 197.5 | 252.9 | 350.0 | 476.8 | 635.3 |
