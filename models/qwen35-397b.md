...
b12x is the fastest MoE backend at low concurrency (1-2 users). For high concurrency (4+), switch to cutlass:
...
| Image | Purpose |
|---|---|
| **`voipmonitor/sglang:cu130`** | **Recommended.** SGLang with SM120 patches, FlashInfer from source (PR #2913 GDC), b12x MoE backend, PCIe allreduce, NCCL graph. PyTorch 2.11 cu130. |
...
### Decode throughput — MTP OFF, 4x RTX PRO 6000, TP=4 (March 2026)

**SGLang b12x** (`--moe-runner-backend b12x`):
...
\* conc=64 capped at 47-48/64 active requests on SGLang

**Summary:**

| Engine | Best for | Peak tok/s | Single-user (conc=1) |
|--------|----------|-----------|---------------------|
| SGLang b12x | conc 1-2 | 1030 (c=64) | **92 tok/s** |
| SGLang cutlass | conc 4-32 | 1230 (c=64) | 70 tok/s |
| vLLM | conc 32+ | 2277 (c=128) | 72 tok/s |

b12x uses a single fused kernel (1 launch) vs 7+ separate kernels for cutlass — dominates at low concurrency. Cutlass scales 1.73× better at conc=16. vLLM handles conc=128 where SGLang caps at ~48-64.
...
| 397B NVFP4 | SGLang b12x + PCIe oneshot | No | 98.4 | PCIe oneshot allreduce, March 2026 |
