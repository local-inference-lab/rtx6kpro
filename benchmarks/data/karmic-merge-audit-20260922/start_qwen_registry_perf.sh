...
registry_gpus=${QWEN_REGISTRY_GPUS:-5,6}
registry_port=${QWEN_REGISTRY_PORT:-5071}
docker run -d --name "$registry_container" \
  --init --gpus "\"device=${registry_gpus}\"" --network host --ipc host --shm-size 32g \
  ...
  -e TP=2 -e DCP=1 -e PORT="$registry_port" -e MAX_MODEL_LEN=524288 \
  ...
  --profiler-config.torch_profiler_with_stack "${QWEN_PROFILE_STACK:-false}" \
